          ...
          {assignments.map((assignment) => {
            ...
            const header = (
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                ...
                {assignment.status === "VERIFIED" && <Icon name="check" width={20} height={20} className="text-signal-success shrink-0" />}
              </div>
            );
            return (
              <CollapsibleCard key={assignment.id} header={header} defaultCollapsed={assignment.status === "VERIFIED"}>
