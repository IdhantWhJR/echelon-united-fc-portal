...
export default async function PaymentsPage() {
  ...
  const finesOwed = fines.filter((f) => f.status === "UNPAID");
  const finesOwedTotal = finesOwed.reduce((sum, f) => sum + f.amountMinor, 0);
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="mt-10">
        <div className="mb-4">
          <p className="eyebrow mb-1">Club</p>
          <h2 className="font-display text-xl font-bold text-paper">Fines</h2>
          <p className="mt-1 text-sm text-paper-faint">Fines assigned to you by your coaching staff.</p>
        </div>

        {finesOwed.length > 0 && (
          <div className="card mb-5 flex items-center justify-between border-signal-danger/30 bg-signal-danger/5 p-4">
            <div>
              <p className="eyebrow mb-1 text-signal-danger">Total owed</p>
              <p className="stat-figure text-xl text-paper">
                {(finesOwedTotal / 100).toFixed(2)} {finesOwed[0]?.currency ?? "GBP"}
              </p>
            </div>
            <Icon name="alert" width={22} height={22} className="text-signal-danger" />
          </div>
        )}
        ...
        {fines.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-line px-6 py-12 text-center">
            <Icon name="check" width={20} height={20} className="mb-3 text-paper-faint" />
            <p className="text-sm font-medium text-paper">No fines on record.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {fines.map((f) => (
              <div key={f.id} className="card flex items-center justify-between gap-4 p-4">
                <div>
                  <p className="font-display text-base font-bold text-paper">{f.reason}</p>
                  <p className="mt-0.5 text-xs text-paper-faint">{format(f.issuedAt, "d MMM yyyy")}</p>
                  {f.note && <p className="mt-1 text-xs text-paper-faint">{f.note}</p>}
                </div>
                <div className="flex flex-col items-end gap-1.5">
                  <p className="font-mono text-sm text-paper">
                    {(f.amountMinor / 100).toFixed(2)} {f.currency}
                  </p>
                  <span className={`badge ${FINE_STATUS_STYLES[f.status]}`}>{f.status}</span>
                </div>
              </div>
            ))}
          </div>
