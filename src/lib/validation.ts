...
export const customMetricEntrySchema = z.object({
  playerProfileId: z.string().cuid().optional(), // omitted when a player logs their own
  metricName: z.string().min(1, "Give the metric a name").max(60),
...
export const adminUpdatePlayerSchema = z.object({
  ...
  salaryEligible: z.boolean().optional(),
});
...
// Staff marking a workout as "attended" from the attendance page. Reuses
// WorkoutStatus as the attendance proxy (VERIFIED = present) rather than
// adding a new field — ASSIGNED is used as the "not present" reset state
// since it's the default un-actioned status.
export const workoutAttendanceUpdateSchema = z.object({
  attended: z.boolean(),
});
...
export const fineCreateSchema = z.object({
  playerProfileId: z.string().cuid(),
  reason: z.string().min(2, "Reason is too short").max(200),
  amount: z.number().positive("Amount must be greater than 0").max(100000),
  currency: z.string().length(3).default("GBP"),
  note: z.string().max(500).optional(),
});

export const fineStatusUpdateSchema = z.object({
  status: fineStatusSchema,
});

export const salaryEligibilityUpdateSchema = z.object({
  salaryEligible: z.boolean(),
});
