...
export function WorkoutForm({ squads, players }: { squads: Squad[]; players: Player[] }) {
  ...
  function toggleExerciseCollapsed(index: number) {
    setCollapsedExercises((current) => ({ ...current, [index]: !current[index] }));
  }

  function addExercise() {
    setExercises((current) => [...current, emptyExercise()]);
    // The new card renders after this tick, so wait a frame before scrolling
    // to it — otherwise it doesn't exist in the DOM yet to scroll to.
    requestAnimationFrame(() => {
      lastExerciseRef.current?.scrollIntoView({ behavior: "smooth", block: "center" });
      lastExerciseRef.current?.querySelector("input")?.focus();
    });
  }
    ...
    <form onSubmit={submit} className="card space-y-5 p-5 lg:p-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          {formCollapsed && title && <p className="text-xs leading-relaxed text-paper-faint">{title}</p>}
        </div>
        <button
          type="button"
        ...
        >
          <Icon name={formCollapsed ? "chevronDown" : "chevronUp"} width={13} height={13} />
          {formCollapsed ? "Expand" : "Minimize"}
        </button>
      </div>
      {formCollapsed ? null : (
      <>
      ...
      <div>
        <div className="mb-2 flex items-center justify-between">
          ...
          <button type="button" onClick={addExercise} className="btn-secondary px-2.5 py-1.5 text-[10px]">
            <Icon name="plus" width={13} height={13} /> Add exercise
          </button>
        </div>
        <div className="space-y-3">
          {exercises.map((exercise, index) => {
            const isCollapsed = !!collapsedExercises[index];
            return (
            <div
              key={index}
              ref={index === exercises.length - 1 ? lastExerciseRef : undefined}
              className="rounded-md border border-line bg-ink-900 p-3 scroll-mt-24"
            >
              <div className={isCollapsed ? "flex items-center justify-between" : "mb-3 flex items-center justify-between"}>
                <button
                  type="button"
                ...
                >
                  <Icon name={isCollapsed ? "chevronDown" : "chevronUp"} width={13} height={13} className="text-paper-faint shrink-0" />
                  <span className="font-display text-xs font-semibold uppercase tracking-wide text-gold">Exercise {index + 1}</span>
                  {isCollapsed && exercise.name && <span className="truncate text-xs font-normal normal-case text-paper-faint">· {exercise.name}</span>}
                </button>
                {exercises.length > 1 && (
                  <button
                    type="button"
              ...
              {!isCollapsed && (
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
                  <div className="sm:col-span-2 lg:col-span-2">
                    <label className="mb-1 block text-[10px] uppercase tracking-wide text-paper-faint">Exercise name</label>
                    <input required className="input-field" value={exercise.name} onChange={(e) => updateExercise(index, "name", e.target.value)} placeholder="e.g. Barbell squat" />
                  </div>
                  <div>
                    <label className="mb-1 block text-[10px] uppercase tracking-wide text-paper-faint">Sets</label>
                    <input type="number" min="1" className="input-field" value={exercise.sets} onChange={(e) => updateExercise(index, "sets", e.target.value)} placeholder="e.g. 4" />
                  </div>
                  <div>
                    <label className="mb-1 block text-[10px] uppercase tracking-wide text-paper-faint">Reps / time</label>
                    <input className="input-field" value={exercise.reps} onChange={(e) => updateExercise(index, "reps", e.target.value)} placeholder="e.g. 10 or 30s" />
                  </div>
                  <div>
                    <label className="mb-1 block text-[10px] uppercase tracking-wide text-paper-faint">Duration (sec)</label>
                    <input type="number" min="1" className="input-field" value={exercise.durationSec} onChange={(e) => updateExercise(index, "durationSec", e.target.value)} placeholder="e.g. 60" />
                  </div>
                  <div>
                    <label className="mb-1 block text-[10px] uppercase tracking-wide text-paper-faint">Rest (sec)</label>
                    <input type="number" min="0" className="input-field" value={exercise.restSec} onChange={(e) => updateExercise(index, "restSec", e.target.value)} placeholder="e.g. 90" />
                  </div>
                </div>
              )}
            </div>
